...
from models.EvaluationResultBaseModel import EvaluationResultBaseModel
load_dotenv()
...
def store_customer_document():
    ...
    customer_result = Chroma.from_documents(
        documents=customer_docs,
        embedding=embeddings,
        persist_directory="./db/customer_db"
    )
...
def store_company_document():
    ...
    company_result = Chroma.from_documents(
        documents=company_docs,
        embedding=embeddings,
        persist_directory="./db/company_db"
    )
...
def store_evaluation(evaluation_text):
    ...
    vector_store = Chroma(
        persist_directory="./db/evaluation_db",
...
def retrieve_docs(query, action):
    vector_store = Chroma(
        persist_directory=f"./db/{action}_db",
        embedding_function=embeddings
    )
